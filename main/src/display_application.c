...
void display_task(void *arg){
    ...
    while(1){
        ...
        //Every frame gets a fresh re-draw
        ssd1309_clear();

        bool music_playing = mp3_is_music_playing();
        int index_songs = songs_get_index();

        //Push static labels and objects to display buffer
        update_display_info(wifi_text, time_text, time_ampm_text, alarm_hour_text, alarm_minute_text,
                                alarm_ampm_text, sleep_text, index_text, index_songs);

        //special case for large screensaver text
        ...
        //Snooze wave will partially wake display to show large clock time
        if (wake_type == WAKE_PARTIAL) {
            ssd1309_draw_big_text(4, 8, large_time_text);
            ssd1309_draw_text(112, 6, time_ampm_text);
        }

        else{
            //draw regular interface during user operation
            ...
            switch (alarm_state_snapshot){
                ...
                case  ALARM_SNOOZED:
                    ...
                    ssd1309_draw_text(12, 4, "Snooze:  mins");
                    snprintf(snooze_text, 32, "%3ld", snooze_time_display);
                    ssd1309_draw_text(54, 4, snooze_text);
                    break;
...
//checks current values and formats values into chars for pushing to display
void update_display_info(char *wifi_text, char *time_text, char *time_ampm_text, char *alarm_hour_text,
    ...
    snprintf(wifi_text, 32, "%s",
        (wifi_is_connected() && wifi_time_is_synced()) ? "OK " : "N/A");
    ...
    //time sync failed, clock time not trusted
    else{
        snprintf(time_text, 32, "--:--");
    }

    snprintf(time_ampm_text, 32, "%s", &time_text[6]);
    snprintf(alarm_hour_text, 32, "%2d", display_alarm_hour);
    snprintf(alarm_minute_text, 32, "%02d", alarm_min);
    snprintf(alarm_ampm_text, 32, "%s", alarm_ampm);    
    snprintf(sleep_text, 32, "%4.1f", display_sleep_hours);
    snprintf(index_text, 32, "%d", index_songs);
    
}

//progress bar loading animation, currently everything but the WiFi is on measured times
...
void screen_saver(enum alarm state, uint32_t *wake_type){
    ...
    //Display Brightness Change Conditions, conditions broken out to make logic clearer
